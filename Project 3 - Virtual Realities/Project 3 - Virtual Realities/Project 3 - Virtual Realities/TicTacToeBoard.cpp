...
bool TicTacToeBoard::IsFinished() const {
	auto lPositions = BoardPosition::GetRectangularPositions(BOARD_SIZE, BOARD_SIZE);
	auto lPossMoves = GetPossibleMoves();
	if (lPossMoves.empty()) {
		return true;
	}
	for (BoardPosition p : lPositions) {
...
bool TicTacToeBoard::isRow(BoardPosition pPosition, BoardDirection pDirection) const{ 
	int count = 0;
...
BoardPosition TicTacToeBoard::PartOfRow() const {
	auto lPositions = BoardPosition::GetRectangularPositions(BOARD_SIZE, BOARD_SIZE);
	for (BoardPosition p : lPositions) {
		for (BoardDirection d : BoardDirection::CARDINAL_DIRECTIONS) {
			if (isRow(p, d)) {
